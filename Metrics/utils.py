...
# def one_hot2dist(seg: np.ndarray, resolution: Tuple[float, float] = None,
...
def one_hot2dist2(seg: np.ndarray, resolution: Tuple[float, float] = None, dtype=None) -> np.ndarray:
    assert one_hot(torch.tensor(seg), axis=0)
    batch_size,K,height,width = seg.shape
    res = np.zeros_like(seg, dtype=dtype)
    for k in range(K):
        for b in range(batch_size):
            posmask = seg[b,k].astype(np.bool_)
            if posmask.any():
                negmask = ~posmask
                res[b,k] = eucl_distance(negmask, sampling=resolution) * negmask \
                    - (eucl_distance(posmask, sampling=resolution) - 1) * posmask[b]
                
    print(res.shape)

    return res

def one_hot2dist(seg: np.ndarray, resolution: Tuple[float, float] = None, dtype=None) -> np.ndarray:
    assert one_hot(torch.tensor(seg), axis=0)
    batch_size, K, height, width = seg.shape
    res = np.zeros_like(seg, dtype=dtype)
    ...
    for k in range(K):
        for b in range(batch_size):
            ...
            if posmask.any():
                negmask = ~posmask
                print(f"Processing batch {b}, class {k}")
                print(f"posmask.shape: {posmask.shape}")
                print(f"negmask.shape: {negmask.shape}")
                
                # Make sure eucl_distance is called with correct arguments
                dist_neg = eucl_distance(negmask, sampling=resolution)
                dist_pos = eucl_distance(posmask, sampling=resolution)
                
                print(f"dist_neg.shape: {dist_neg.shape}")
                print(f"dist_pos.shape: {dist_pos.shape}")

                if np.isnan(dist_neg).any() or np.isinf(dist_neg).any() or np.isnan(dist_pos).any() or np.isinf(dist_pos).any():
                    print(f"NaNs or Infs found in distance maps")

                res[b, k] = dist_neg * negmask - (dist_pos - 1) * posmask
...
def one_hot_encode(label, n_classes, requires_grad=True):
    ...
    print(label.min(), label.max())  # Check min and max values
    if not (label >= 0).all() or not (label < n_classes).all():
        raise ValueError(f"Labels must be in the range [0, {n_classes - 1}]")

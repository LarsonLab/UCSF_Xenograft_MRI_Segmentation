...
def train(model_name,model,optimizer,criterion,train_loader,val_loader,device,num_epochs,clear_mem):
    ...
    print(f'Model sent to '+str(device))
    ...
    for epoch in range(num_epochs): 
        ...
        for i, batch in tqdm(enumerate(train_loader),total = len(train_loader), desc=f"Training Epoch:{epoch+1}/{num_epochs}"): 
            ...
            # train_scores.append(spec_loss(output.detach(),msk).item())
            # print(dice_train_losses)
            iters += 1
            if iters % 100 == 0:
                torch.cuda.empty_cache()

        all_dice_train_losses.append(sum(dice_train_losses)/len(dice_train_losses))
        # all_train_scores.append(sum(train_scores)/len(train_scores))     
        ...
        #   val_scores.append(spec_loss(output.detach(),msk).item())
          dice_val_preds.append(output.cpu())
          dice_val_labels.append(msk.cpu())
...
# def visualize_segmentation(model,data_loader,num_samples,device):
...
#                 (1,2,0)).astype(np.uint8),cmap='gray',interpolation='none')
